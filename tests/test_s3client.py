...
class S3ClientTest(BaseMRRCTest):
    ...
    def test_upload_and_delete_files(self):
        ...
        # Second, test upload existed files with the product.
        # The product will be added to metadata
        self.s3_client.upload_files(
            all_files, bucket_name=MY_BUCKET, product="apache-commons", root=root
        )
        objects = list(bucket.objects.all())
        self.assertEqual(26, len(objects))
        for obj in objects:
        ...
        # Third, test upload existed files with extra product.
        # The extra product will be added to metadata
        self.s3_client.upload_files(
            all_files, bucket_name=MY_BUCKET, product="commons-lang3", root=root
        )
        objects = list(bucket.objects.all())
        self.assertEqual(26, len(objects))
        for obj in objects:
        ...
        # Fourth, test delete files without product.
        # The file will not be deleted and no product metadata will be changed.
        ...
        # Fifth, test delete files with one prodct.
        # The file will not be deleted, but the product will
        ...
        # Finally, test delete files with left prodct.
        # The file will be deleted, because all products have been
