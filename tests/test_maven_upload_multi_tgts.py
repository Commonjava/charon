...
class MavenUploadMultiTgtsTest(PackageBaseTest):
    ...
    def test_short_prefix_upload(self):
        self.__test_prefix_upload(
            [
                (None, TEST_BUCKET, SHORT_TEST_PREFIX),
                (None, TEST_BUCKET_2, SHORT_TEST_PREFIX, None),
            ]
        )
    ...
    def test_long_prefix_upload(self):
        self.__test_prefix_upload(
            [
                (None, TEST_BUCKET, LONG_TEST_PREFIX),
                (None, TEST_BUCKET_2, LONG_TEST_PREFIX, None),
            ]
        )
    ...
    def test_overlap_upload(self):
        ...
        for target in targets_:
            ...
            self.assertEqual(expected, len(actual_files), msg=f"{bucket_name}")
            ...
            self.assertIn("<version>4.5.6</version>", cat_content, msg=f"{bucket_name}")
            self.assertIn("<version>4.5.9</version>", cat_content, msg=f"{bucket_name}")
            self.assertIn(
                "<artifactId>httpclient</artifactId>", cat_content, msg=f"{bucket_name}"
            )
            self.assertIn(
                "<groupId>org.apache.httpcomponents</groupId>",
                cat_content,
    ...
    def __test_prefix_upload(self, targets: List[Tuple[str, str, str, str]]):
        ...
        handle_maven_uploading(
            test_zip, product, targets=targets, dir_=self.tempdir, do_index=False
        )
        ...
        for target in targets:
            ...
            self.assertEqual(expected, len(actual_files), msg=f"{bucket.name}")
            ...
            self.assertIn(
                "<version>4.5.6</version>", meta_content_client, msg=f"{bucket.name}"
            )
            self.assertIn(
                "<latest>4.5.6</latest>", meta_content_client, msg=f"{bucket.name}"
            )
            self.assertIn(
                "<release>4.5.6</release>", meta_content_client, msg=f"{bucket.name}"
            )
            self.assertNotIn(
                "<version>4.5.9</version>", meta_content_client, msg=f"{bucket.name}"
            )

            meta_obj_logging = self.test_bucket.Object(
                PREFIXED_COMMONS_LOGGING_METAS[0]
            )
            meta_content_logging = str(meta_obj_logging.get()["Body"].read(), "utf-8")
            self.assertIn(
                "<groupId>commons-logging</groupId>",
            ...
            self.assertIn(
                "<version>1.2</version>", meta_content_logging, msg=f"{bucket.name}"
            )
            self.assertIn(
                "<latest>1.2</latest>", meta_content_logging, msg=f"{bucket.name}"
            )
            self.assertIn(
                "<release>1.2</release>", meta_content_logging, msg=f"{bucket.name}"
            )

            catalog = self.test_bucket.Object(PREFIXED_ARCHETYPE_CATALOG_FILES[0])
            cat_content = str(catalog.get()["Body"].read(), "utf-8")
            self.assertIn("<version>4.5.6</version>", cat_content, msg=f"{bucket.name}")
            self.assertIn(
                "<artifactId>httpclient</artifactId>", cat_content, msg=f"{bucket.name}"
            )
            self.assertIn(
                "<groupId>org.apache.httpcomponents</groupId>",
                cat_content,
