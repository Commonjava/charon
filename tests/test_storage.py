...
class S3ClientTest(BaseMRRCTest):
    ...
    def test_upload_and_delete_files(self):
        ...
        os.mkdir(temp_root)
        extract_zip_all(zipfile.ZipFile(os.path.join(os.getcwd(), 'tests/input/commons-lang3.zip')),
                        temp_root)
    ...
    def test_upload_file_with_checksum(self):
        ...
        self.s3_client.upload_files([file], bucket_name=MY_BUCKET, product="foo-bar-1.0",
                                    root=temp_root)
        objects = list(bucket.objects.all())
        ...
        self.s3_client.upload_files([file], bucket_name=MY_BUCKET, product="foo-bar-1.0-2",
                                    root=temp_root)
        objects = list(bucket.objects.all())
        self.assertEqual(1, len(objects))
        obj = objects[0].Object()
    ...
    def test_upload_metadata_with_checksum(self):
        ...
        write_file(file, content1)
        sha1_1 = read_sha1(file)
        self.s3_client.upload_metadatas([file], bucket_name=MY_BUCKET, product="foo-bar-1.0",
                                        root=temp_root)
        objects = list(bucket.objects.all())
        self.assertEqual(1, len(objects))
        obj = objects[0].Object()
        ...
        self.s3_client.upload_metadatas([file], bucket_name=MY_BUCKET,
                                        product="foo-bar-1.0-repeated", root=temp_root)
        objects = list(bucket.objects.all())
        self.assertEqual(1, len(objects))
        obj = objects[0].Object()
        self.assertEqual(set("foo-bar-1.0,foo-bar-1.0-repeated".split(",")),
        ...
        sha1_2 = read_sha1(file)
        self.assertNotEqual(sha1_1, sha1_2)
        self.s3_client.upload_metadatas([file], bucket_name=MY_BUCKET, product="foo-bar-1.0-2",
                                        root=temp_root)
        objects = list(bucket.objects.all())
        self.assertEqual(1, len(objects))
        obj = objects[0].Object()
        self.assertEqual(set("foo-bar-1.0,foo-bar-1.0-2,foo-bar-1.0-repeated".split(",")),
                         set(obj.metadata[PRODUCT_META_KEY].split(",")))
