...
class S3Client(object):
    ...
    def upload_files(self, file_paths: List[str], bucket_name=None, product=None, root="/"):
        ...
        def path_upload_handler(full_file_path: str, path: str) -> bool:
            ...
            if not existed:
                ...
                logger.info('File %s already exists, check if need to update product.',
                            full_file_path)
                ...
                if product not in prods:
                    logger.info('File %s has new product, updating the product %s', full_file_path,
                                product)
    ...
    def upload_metadatas(self, meta_file_paths: List[str], bucket_name=None, product=None,
                         root="/"):
        ...
        def path_upload_handler(full_file_path: str, path: str) -> bool:
            ...
            if existed:
                f_meta = fileObject.metadata
                need_overwritten = CHECKSUM_META_KEY not in f_meta or sha1 != f_meta[
                    CHECKSUM_META_KEY]
        ...
        def path_delete_handler(full_path: str, path: str) -> bool:
            ...
            if existed:
                ...
                if product and product in prods:
                    ...
                    if len(prods) > 0:
                        logger.info('File %s has other products overlapping, will remove %s from '
                                    'its metadata', path, product)
                        self.__update_file_metadata(fileObject, bucket_name, path,
                                                    {PRODUCT_META_KEY: ",".join(prods)})
            if len(prods) == 0:
